...
function secure_document_init() {
	...
	$secure_doc_label = apply_filters( 'goliath-secure-docs-cpt-secure-document-label',
		array(
			'name'               => __( 'Secure documents',
				'goliath-secure-docs' ),
			'singular_name'      => __( 'Secure document',
				'goliath-secure-docs' ),
			'all_items'          => __( 'All Secure documents',
				'goliath-secure-docs' ),
			'new_item'           => __( 'New Secure document',
				'goliath-secure-docs' ),
			'add_new'            => __( 'Add New', 'goliath-secure-docs' ),
			'add_new_item'       => __( 'Add New Secure document',
				'goliath-secure-docs' ),
			'edit_item'          => __( 'Edit Secure document',
				'goliath-secure-docs' ),
			'view_item'          => __( 'View Secure document',
				'goliath-secure-docs' ),
			'search_items'       => __( 'Search Secure documents',
				'goliath-secure-docs' ),
			'not_found'          => __( 'No Secure documents found',
				'goliath-secure-docs' ),
			'not_found_in_trash' => __( 'No Secure documents found in trash',
				'goliath-secure-docs' ),
			'parent_item_colon'  => __( 'Parent Secure document',
				'goliath-secure-docs' ),
			'menu_name'          => __( 'Secure docs', 'goliath-secure-docs' ),
		) );

	$secure_doc_args = apply_filters( 'goliath-secure-docs-cpt-secure-document-params',
		array(
...
function secure_document_updated_messages( $messages ) {
	...
	$messages['secure-document'] = array(
		...
		1  => sprintf( __( 'Secure document updated. <a target="_blank" href="%s">View Secure document</a>',
			'goliath-secure-docs' ), esc_url( $permalink ) ),
		...
		5  => isset( $_GET['revision'] ) ? sprintf( __( 'Secure document restored to revision from %s',
			'goliath-secure-docs' ),
			wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		6  => sprintf( __( 'Secure document published. <a href="%s">View Secure document</a>',
			'goliath-secure-docs' ), esc_url( $permalink ) ),
		7  => __( 'Secure document saved.', 'goliath-secure-docs' ),
		8  => sprintf( __( 'Secure document submitted. <a target="_blank" href="%s">Preview Secure document</a>',
			'goliath-secure-docs' ),
			esc_url( add_query_arg( 'preview', 'true', $permalink ) ) ),
		9  => sprintf( __( 'Secure document scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Secure document</a>',
			'goliath-secure-docs' ),
			// translators: Publish box date format, see http://php.net/date
			date_i18n( __( 'M j, Y @ G:i', 'goliath-secure-docs' ),
				strtotime( $post->post_date ) ), esc_url( $permalink ) ),
		10 => sprintf( __( 'Secure document draft updated. <a target="_blank" href="%s">Preview Secure document</a>',
			'goliath-secure-docs' ),
			esc_url( add_query_arg( 'preview', 'true', $permalink ) ) ),
	);
...
function secure_document_register_file_meta_box() {
	add_meta_box( 'secure_document_file_meta_box',
		__( 'Secure file', 'goliath-secure-docs' ),
		'secure_document_file_meta_box_content', 'secure-document' );
}
...
function secure_document_file_meta_box_content( $post ) {
	...
	$secure_doc_path_meta = get_post_meta( $post->ID, '_secure_doc_path',
		true );
	$secure_doc_mime_meta = get_post_meta( $post->ID, '_secure_doc_mime_type',
		true );
    ...
    <input type="file"
           name="goliath_secure_doc_file" <?php echo $required_attribut ?>/>
	<?php
...
add_action( 'save_post_secure-document',
	'goliath_secure_documents_save_file_and_meta' );

function goliath_secure_documents_save_file_and_meta( $post_id ) {
	if ( isset( $_FILES['goliath_secure_doc_file'] ) && check_admin_referer( 'secure_document_nonce',
			'secure_document_nonce_name' )
	) {
		...
		if ( is_uploaded_file( $secure_doc_file['tmp_name'] ) ) {
			...
			$name = sanitize_file_name( $secure_doc_file['name'] );

			// get extension
			preg_match( '/\.[^\.]+$/i', $name, $ext );
			$name_with_no_ext = substr( $name, 0, - ( strlen( $ext[0] ) ) );
			$name             = sanitize_title( $name_with_no_ext ) . $ext[0];

			// The file new path
			$secure_doc_path = goliath_secure_documents_get_docs_folder() . '/' . $name;
			...
			if ( is_file( $secure_doc_path ) ) {
				...
				$secure_doc_path = substr( $secure_doc_path, 0,
						- ( strlen( $ext[0] ) ) ) . '_' . time() . $ext[0];
			}

			// Save the file in the write place
			$file_is_move = move_uploaded_file( $secure_doc_file['tmp_name'],
				$secure_doc_path );

			// Update post meta
			if ( $file_is_move ) {

				update_post_meta( $post_id, '_secure_doc_path',
					basename( $secure_doc_path ) );
				update_post_meta( $post_id, '_secure_doc_mime_type',
					$secure_doc_file['type'] );
			}
		}
	}
}
...
add_filter( 'manage_secure-document_posts_columns',
	'goliath_secure_doc_column' );
...
add_action( 'manage_secure-document_posts_custom_column',
	'goliath_secure_doc_column_content', 10, 2 );
...
function goliath_secure_doc_column_content( $column_name, $post_id ) {
	...
	if ( $column_name == 'secure_doc_link' ) {
		$secure_doc_path_meta = get_post_meta( $post_id, '_secure_doc_path',
			true );
